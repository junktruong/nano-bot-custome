...
def _make_provider(config: Config):
    ...
    # ChatGPT Web via Playwright (no API key required)
    if provider_name == "chatgpt_web":
        web_cfg = config.providers.chatgpt_web
        return ChatGPTWebProvider(
            default_model=model,
            chat_url=web_cfg.chat_url,
...
def cron_list(
    ...
    rtc_tz_name = get_rtc_timezone_name()
    ...
    for job in jobs:
        ...
        if job.state.next_run_at_ms:
            ...
            try:
                tz = ZoneInfo(job.schedule.tz) if job.schedule.tz else rtc_tz
...
def cron_add(
    ...
    if every:
        ...
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=get_rtc_zoneinfo())
...
def status():
    ...
    if config_path.exists():
        ...
        chatgpt_web = config.providers.chatgpt_web
        console.print(
            f"ChatGPT Web: [green]✓[/green] {chatgpt_web.chat_url} "
            f"(profile: {chatgpt_web.user_data_dir})"
        )

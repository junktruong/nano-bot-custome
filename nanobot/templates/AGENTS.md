...
cron(action="add", message="Nhắc lịch buổi sáng", cron_expr="0 5 * * *")
cron(action="add", message="Nhắc mỗi giờ", every_hours=1)
cron(action="add", message="Nhắc học", hour=20, minute=0, weekdays="mon,wed,fri")
cron(action="update", job_id="<job_id>", hour=21, minute=30, weekdays="2,4,6")
